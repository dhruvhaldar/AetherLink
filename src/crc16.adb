package body CRC16 with SPARK_Mode is
   ...
   type CRC_Table_Type is array (Unsigned_8) of Unsigned_16;
   ...
   function Update (CRC : Unsigned_16; Byte : Unsigned_8) return Unsigned_16 is
      Index : Unsigned_8;
   begin
      --  Optimized table lookup:
      --  CRC = (CRC << 8) XOR Table((CRC >> 8) XOR Byte)
      Index := Unsigned_8 (Shift_Right (CRC, 8)) xor Byte;
      return Shift_Left (CRC, 8) xor CRC_Table (Index);
   end Update;

   function Compute (Data : in String) return Unsigned_16 is
      CRC : Unsigned_16 := Initial_Value; -- CCITT Initial Value
   begin

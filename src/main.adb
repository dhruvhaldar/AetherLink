...
procedure Main is
   ...
   begin
      ...
      --  Padding for alignment (target 16 bytes width)
      if Length < 16 then
         for K in 1 .. (16 - Length) loop
            Put ("   ");
         end loop;
      end if;
      ...
      for I in 1 .. Length loop
         Byte_Val := Data(Data'First + I - 1);
         if Byte_Val in 32 .. 126 then
            Char_Val := Character'Val(Byte_Val);
            Put (Char_Val);
         else
            Put ('.');
         end if;
